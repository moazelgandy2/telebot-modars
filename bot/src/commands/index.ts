...
import { isBotActive, getReplyTarget, isWithinWorkingHours, getResponseDelay } from "../utils/settings.js";
...
const processAggregatedMessage = async (client: TelegramClient, userId: string, aggregatedText: string, latestEvent: NewMessageEvent) => {
  ...
  try {
    ...
    if (faqAnswer) {
      // Queue Response
      const delay = await getResponseDelay();
      ...
      await fetch(`${config.apiBaseUrl}/response-queue`, {
        ...
        body: JSON.stringify({
          ...
          message: faqAnswer,
          replyToMsgId: latestEvent.message.id,
