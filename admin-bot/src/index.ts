...
bot.action("menu_system", async (ctx) => {
  try {
    ...
    const { botActive, replyTarget, responseDelay } = res.data.data || {};
    const isActive = botActive !== 'false';
    const isSubOnly = replyTarget === 'subscribers';
    const delay = responseDelay || "0";
    ...
    await ctx.editMessageText(msg, {
      ...
      ...Markup.inlineKeyboard([
        ...
        [Markup.button.callback("⏳ ضبط التأخير", "set_delay_start"), Markup.button.callback("🕰️ ساعات العمل", "hours_view")],
        [BackToMainBtn]
      ])
    });
  } catch (e) {
...
bot.action("set_delay_start", (ctx) => {
  setState(ctx.from!.id, { action: 'WAITING_SET_DELAY' });
  ctx.editMessageText("⏳ **ضبط تأخير الرد**\n\nاكتب عدد الثواني اللي عايز البوت يستناها قبل ما يرد (مثلاً: 5):", {
    parse_mode: "Markdown",
    ...Markup.inlineKeyboard([[Markup.button.callback("إلغاء ❌", "menu_system")]])
  });
});
...
bot.on("text", async (ctx) => {
  ...
  if (state.action === 'WAITING_SET_DELAY') {
    const seconds = parseInt(text, 10);
    if (isNaN(seconds) || seconds < 0) {
      await ctx.reply("⚠️ من فضلك اكتب رقم صحيح (ثواني).");
      return;
    }
    ...
    try {
      const getRes = await axios.get(`${config.apiBaseUrl}/settings`);
      const currentSettings = getRes.data.data || {};
      await axios.post(`${config.apiBaseUrl}/settings`, { ...currentSettings, responseDelay: String(seconds) });
      await ctx.reply(`✅ تم ضبط التأخير إلى ${seconds} ثانية.`, { parse_mode: "Markdown", ...Markup.inlineKeyboard([[BackToMainBtn]]) });
      clearState(userId);
    } catch (e) {
      console.error("Set Delay Error:", e);
      await ctx.reply("❌ حصل مشكلة وأنا بحفظ الإعدادات.");
    }
    return;
  }
});
